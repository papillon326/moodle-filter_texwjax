 ...
 */
    ...
    if ($action=='ShowDB' or $action=='DeleteDB') {
        $md5 = md5($texexp);
        $texcache = $DB->get_record("cache_filters", array("filter"=>"texwjax", "md5key"=>$md5));
    }
    ...
    function TexOutput($expression, $graphic=false) {
        ...
        $pathlatex = trim(get_config('filter_texwjax', 'pathlatex'), " '\"");
