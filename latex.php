<?php
    ...
    class latex {
        ...
        function construct_latex_document( $formula, $fontsize=12 ) {
            ...
            $formula = filter_texwjax_sanitize_formula($formula);

            // $fontsize don't affects to formula's size. $density can change size
            $doc =  "\\documentclass[{$fontsize}pt]{article}\n";
            $doc .= get_config('filter_texwjax', 'latexpreamble');
            $doc .= "\\pagestyle{empty}\n";
            $doc .= "\\begin{document}\n";
//dlnsk            $doc .= "$ {$formula} $\n";
...
//                return false;
            ...
            // run dvips (.dvi to .ps)
            $pathdvips = escapeshellarg(trim(get_config('filter_texwjax', 'pathdvips'), " '\""));
            ...
            if ($convertformat == 'svg') {
                $pathdvisvgm = escapeshellarg(trim(get_config('filter_texwjax', 'pathdvisvgm'), " '\""));
                $command = "$pathdvisvgm -E $ps -o $img";
            } else {
                $pathconvert = escapeshellarg(trim(get_config('filter_texwjax', 'pathconvert'), " '\""));
