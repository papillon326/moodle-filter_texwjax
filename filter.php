 ...
 */
function filter_text_image_wjax($imagefile, $tex, $height, $width, $align, $alt) {
    global $CFG, $OUTPUT;
    ...
    if ($CFG->slasharguments) {        // Use this method if possible for better caching
        $anchorcontents .= "$CFG->wwwroot/filter/texwjax/pix.php/$imagefile";
    } else {
        $anchorcontents .= "$CFG->wwwroot/filter/texwjax/pix.php?file=$imagefile";
    }
    $anchorcontents .= "\" $style/>";

    if (!file_exists("$CFG->dataroot/filter/texwjax/$imagefile") && has_capability('moodle/site:config', context_system::instance())) {
        $link = '/filter/texwjax/texdebug.php';
        $action = null;
    } else {
        $link = new moodle_url('/filter/texwjax/displaytex.php', array('texexp'=>$tex));
 ...
 * TeX filtering class.
 */
class filter_texwjax extends moodle_text_filter {
