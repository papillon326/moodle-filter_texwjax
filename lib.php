...
function filter_texwjax_get_executable($debug=false) {
    ...
    $error_message1 = "Your system is not configured to run mimeTeX. You need to download the appropriate<br />"
                     ...
                     ."put the executable into your<br /> moodle/filter/texwjax/ directory.";
    ...
    switch (PHP_OS) {
        case "Linux":   return "$CFG->dirroot/filter/texwjax/mimetex.linux";
        case "Darwin":  return "$CFG->dirroot/filter/texwjax/mimetex.darwin";
        case "FreeBSD": return "$CFG->dirroot/filter/texwjax/mimetex.freebsd";
    }

    print_error('mimetexisnotexist', 'error');
}

function filter_texwjax_sanitize_formula($texexp) {
...
function filter_texwjax_get_cmd($pathname, $texexp) {
    $texexp = filter_texwjax_sanitize_formula($texexp);
    $texexp = escapeshellarg($texexp);
    $executable = filter_texwjax_get_executable(false);
